...
#%% Importar o dataset
### fonte: https://www.kaggle.com/datasets/vetrirah/customer
...
def f_test(kmeans_model, dataframe):
    ...
    for variavel in variaveis:
        ...
        variabilidade_entre_grupos = np.sum([dic[index]*np.square(observacao - media) for index, observacao in enumerate(centroides[variavel])])/(qnt_clusters - 1)
...
# Plotando um gráfico de dispersão com 3 variáveis incluindo as coordenadas do mapa perceptual

fig = px.scatter_3d(df_quanti, x='coord_x', y='coord_y',z='Family_Size', color='cluster_kmeans')
fig.show()
